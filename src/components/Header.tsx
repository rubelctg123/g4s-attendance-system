    ...
    <header className="bg-white/90 backdrop-blur-md border-b border-slate-200 sticky top-0 z-30 shadow-xs">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          {/* User Account & Database Status Indicator */}
          <div className="flex items-center space-x-3">
            {currentUser?.role === 'admin' && (
              <button
                onClick={onOpenSqlModal}
                title={isDbConnected ? 'Connected to Supabase Database' : 'Database Configuration'}
              ...
              >
                ...
                <span className="text-[11px] font-bold">
                  {isDbConnected ? 'Cloud Sync Active' : 'Local Storage Mode'}
                </span>
              ...
              <div className="flex items-center space-x-2 border-l border-slate-200 pl-3">
                <div className="hidden lg:flex flex-col items-end">
                  <span className="text-xs font-bold text-slate-900 leading-none">
                    {currentUser?.name || userEmail?.split('@')[0]}
                  </span>
                  <span className="text-[10px] text-slate-500 font-semibold font-mono mt-0.5">
